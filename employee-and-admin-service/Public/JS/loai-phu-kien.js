...
async function loadData() {
    const res = await fetch('/private/category/loai-phu-kien');
...
document.addEventListener("click", async (e) => {
    ...
    if (e.target.closest(".delete-btn")) {
        if (confirm("Xóa loại phụ kiện này?")) {
            ...
            const res = await fetch(`/private/category/loai-phu-kien/${id}`, { method: 'DELETE' });
            if (res.ok) {
...
document.getElementById("editForm").addEventListener("submit", async (e) => {
    ...
    const res = await fetch(`/private/category/loai-phu-kien/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(Object.fromEntries(formData))
    });
