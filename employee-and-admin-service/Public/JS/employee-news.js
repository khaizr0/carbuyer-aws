...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch('/employee/news');
...
document.getElementById("createNewsForm").addEventListener("submit", async (event) => {
    ...
    try {
        const response = await fetch('/employee/news/create', { method: 'POST', body: formData });
        if (response.ok) {
...
document.addEventListener("click", async (event) => {
    if (event.target.closest(".edit-news")) {
        ...
        try {
            const response = await fetch(`/employee/news/${id}`);
...
document.getElementById("editNewsForm").addEventListener("submit", async (event) => {
    ...
    try {
        const response = await fetch(`/employee/news/${id}`, { method: 'PUT', body: formData });
        if (response.ok) {
...
document.addEventListener("click", async (event) => {
    if (event.target.closest(".delete-news")) {
        ...
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
            try {
                const response = await fetch(`/employee/news/${id}`, { method: 'DELETE' });
                if (response.ok) {
