...
async function loadData() {
    const res = await fetch('/employee/category/thuong-hieu');
...
document.addEventListener("click", async (e) => {
    ...
    if (e.target.closest(".delete-btn")) {
        if (confirm("Xóa thương hiệu này?")) {
            ...
            const res = await fetch(`/employee/category/thuong-hieu/${id}`, { method: 'DELETE' });
            if (res.ok) {
...
document.getElementById("editForm").addEventListener("submit", async (e) => {
    ...
    const res = await fetch(`/employee/category/thuong-hieu/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(Object.fromEntries(formData))
    });
